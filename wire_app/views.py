...
# function ın çalışmadan önce logged in olmayı zorunlu kılıyor.
@login_required(login_url='signin')
def index(request):
    # logged in user'ı verir. (object)
    user_object = User.objects.get(username=request.user.username)
    # logged in user'ın profile ını verir. (object)
...
def upload(request):
    if request.method == 'POST':
        ...
        link = request.POST.get('link_upload')
...
def signup(request):
    if request.method == "POST":
        ...
        if password == '':
            messages.info(request, 'Password is missing')
            ...
            if User.objects.filter(email=email).exists():
                ...
                # Log user in and ridirect to account settings page
